...
const SubjectCard: React.FC<{ name: string; progress: number; gradient: string }> = ({ name, progress, gradient }) => (
  <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-md flex flex-col justify-between transition-shadow hover:shadow-xl">
    ...
    <button className={`w-full text-white font-bold py-3 px-4 rounded-lg bg-gradient-to-r ${gradient} hover:opacity-90 transition-opacity`}>
      Học tiếp
    </button>
  </div>
);
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6">
          <SubjectCard name="Toán học" progress={progress.math} gradient="from-blue-500 to-cyan-500" />
          <SubjectCard name="Ngữ văn" progress={progress.literature} gradient="from-purple-500 to-pink-500" />
          <SubjectCard name="Tiếng Anh" progress={progress.english} gradient="from-green-500 to-lime-500" />
          <SubjectCard name="Tin học" progress={progress.it} gradient="from-orange-500 to-amber-500" />
        </div>
      </div>
    </div>
  );
