  ...
  useEffect(() => {
    try {
      ...
      setChat(chatSession);
        ...
        setMessages([{ role: 'model', text: 'Không thể khởi tạo AI. Vui lòng kiểm tra API key.' }]);
    }
  }, []);
    ...
    try {
      ...
      for await (const chunk of result) {
       ...
       if (error instanceof Error && (error.message.includes("API key not valid") || error.message.includes("provide an API key"))) {
        const errorMessage: Message = { role: 'model', text: 'API key không hợp lệ. Vui lòng kiểm tra tệp .env và làm mới trang.' };
        setMessages(prev => [...prev, errorMessage]);
      } else {
        console.error('Error sending message:', error);
        const errorMessage: Message = { role: 'model', text: 'Rất tiếc, đã có lỗi xảy ra.' };
        setMessages(prev => [...prev, errorMessage]);
      }
    } finally {
      setIsLoading(false);
